/* -*- compile-command: "odin build 1-scale-rotate.odin -file -debug" -*- */
package main
...
vert_src    :: string(#load("main.vert"))
frag_src    :: string(#load("main.frag"))
...
main :: proc() {
    ...
    for !glfw.WindowShouldClose(window) {
        ...
        gl.UseProgram(program)
        trans := glm.mat4(1.0)
        trans *= glm.mat4Translate(glm.vec3{0.5, -0.5, 0.0})
        trans *= glm.mat4Rotate(glm.vec3{0.0, 0.0, 1.0}, f32(glfw.GetTime()))
        gl.UniformMatrix4fv(gl.GetUniformLocation(program, "transform"), 1, false, &trans[0][0])
